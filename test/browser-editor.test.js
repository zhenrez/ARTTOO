...
test('pointer drawing rerenders canonical stroke and controls undo/redo', () => {
  const keyListeners = fakeDocument(); const svg = new FakeElement(); const undo = new FakeElement(); const redo = new FakeElement(); const status = { textContent: '' };
  ...
  undo.emit('click'); assert.equal(svg.children.length, 0); assert.equal(redo.disabled, false); redo.emit('click'); assert.equal(svg.children.length, 1); assert.match(status.textContent, /^Revision /);
  ...
  const keyboardRedo = { ctrlKey: true, metaKey: false, shiftKey: true, key: 'z', preventDefault() { this.prevented = true; } }; keyListeners[0](keyboardRedo); assert.equal(svg.children.length, 1); mounted.destroy(); delete globalThis.document;
});

test('selection exposes numeric transform, scale and flip through reversible canonical commands', () => {
  const keyListeners = fakeDocument(); const svg = new FakeElement(); const controls = transformControls(); let project = fixture();
  project = applyCommand(project, { type: 'stroke.add', artboardId: 'board', objectId: 'stroke-1', points: [{ x: 10, y: 10 }, { x: 20, y: 20 }], style: { preset: 'round', color: '#18151e', width: 1.5, opacity: 1 } });
  ...
  controls.flipX.emit('click'); assert.equal(mounted.host.getProject().objects['stroke-1'].transform.scaleX, -1.5); mounted.host.undo(); assert.equal(mounted.host.getProject().objects['stroke-1'].transform.scaleX, 1.5);
  controls.flipY.emit('click'); assert.equal(mounted.host.getProject().objects['stroke-1'].transform.scaleY, -0.75);
  const right = { ctrlKey: false, metaKey: false, altKey: false, shiftKey: false, key: 'ArrowRight', preventDefault() { this.prevented = true; } }; keyListeners[0](right); assert.equal(right.prevented, true); assert.equal(mounted.host.getProject().objects['stroke-1'].transform.x, 13); mounted.destroy(); delete globalThis.document;
});

test('touch pointer drag previews ephemerally then commits one canonical move that undo restores', () => {
  fakeDocument(); const svg = new FakeElement(); let project = fixture(); project = applyCommand(project, { type: 'stroke.add', artboardId: 'board', objectId: 'stroke-1', points: [{ x: 10, y: 10 }, { x: 20, y: 20 }] });
  const mounted = mountBrowserEditor({ project, artboardId: 'board', svg, undoButton: new FakeElement(), redoButton: new FakeElement() }); const revisionBefore = mounted.host.getProject().revision;
  svg.emit('pointerdown', { pointerId: 7, pointerType: 'touch', clientX: 20, clientY: 20, target: svg.children[0] }); svg.emit('pointermove', { pointerId: 7, pointerType: 'touch', clientX: 60, clientY: 80 });
  assert.equal(mounted.host.getProject().revision, revisionBefore); assert.match(svg.children[0].attributes.transform, /translate\(20 30\)/);
  svg.emit('pointerup', { pointerId: 7, pointerType: 'touch', clientX: 60, clientY: 80 }); assert.equal(mounted.host.getProject().revision, revisionBefore + 1); assert.deepEqual({ x: mounted.host.getProject().objects['stroke-1'].transform.x, y: mounted.host.getProject().objects['stroke-1'].transform.y }, { x: 20, y: 30 });
  mounted.host.undo(); assert.deepEqual({ x: mounted.host.getProject().objects['stroke-1'].transform.x, y: mounted.host.getProject().objects['stroke-1'].transform.y }, { x: 0, y: 0 }); mounted.destroy(); delete globalThis.document;
});

test('touch resize handle previews ephemerally then commits one canonical scale that undo restores', () => {
  fakeDocument(); const svg = new FakeElement(); let project = fixture(); project = applyCommand(project, { type: 'stroke.add', artboardId: 'board', objectId: 'stroke-1', points: [{ x: 10, y: 10 }, { x: 20, y: 20 }] });
  const mounted = mountBrowserEditor({ project, artboardId: 'board', svg, undoButton: new FakeElement(), redoButton: new FakeElement() });
  svg.emit('pointerdown', { pointerId: 3, clientX: 40, clientY: 40, target: svg.children[0] }); svg.emit('pointerup', { pointerId: 3, clientX: 40, clientY: 40 });
  const handle = svg.children.find((child) => child.dataset.resizeObjectId === 'stroke-1'); assert.ok(handle); assert.equal(handle.attributes['aria-label'], 'Resize selected object');
  const revisionBefore = mounted.host.getProject().revision;
  svg.emit('pointerdown', { pointerId: 9, pointerType: 'touch', clientX: 40, clientY: 40, target: handle }); svg.emit('pointermove', { pointerId: 9, pointerType: 'touch', clientX: 60, clientY: 60 });
  assert.equal(mounted.host.getProject().revision, revisionBefore); assert.match(svg.children[0].attributes.transform, /scale\(2 2\)/);
  svg.emit('pointerup', { pointerId: 9, pointerType: 'touch', clientX: 60, clientY: 60 }); assert.equal(mounted.host.getProject().revision, revisionBefore + 1); assert.equal(mounted.host.getProject().objects['stroke-1'].transform.scaleX, 2); assert.equal(mounted.host.getProject().objects['stroke-1'].transform.scaleY, 2);
  mounted.host.undo(); assert.equal(mounted.host.getProject().objects['stroke-1'].transform.scaleX, 1); assert.equal(mounted.host.getProject().objects['stroke-1'].transform.scaleY, 1);
